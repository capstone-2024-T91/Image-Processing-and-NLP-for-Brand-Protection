...
function addButtonToInterface() {
  ...
  if (sortContainer) {
      ...
      sortButton.addEventListener('click', () => {
        ...
        sortButton2.style.animation = 'bg2 1s 0s cubic-bezier(0.6, 0.1, 0.165, 1), bg3 1s 1s cubic-bezier(0.6, 0.1, 0.165, 1)';
...
function classifyEmail(predictions) {
  ...
  if (phishingCount == 5 ) {
      const averageCertainty = phishingCertaintySum / phishingCountWithCertainty;
      return { classification: "Danger", averageCertainty };
  } else if (phishingCount >= 3) {
      const averageCertainty = phishingCertaintySum / phishingCountWithCertainty;
      return { classification: "Moderate", averageCertainty };
  } else {
      const averageCertainty = safeCertaintySum / safeCountwithCertainty;
